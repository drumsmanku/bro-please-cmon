...
function Skill({ skills, directionLeft }: Props) {
  return (
    <div className='group relative flex cursor-pointer'>
      <motion.img
        ...
        src={urlFor(skills?.image).url()} alt="" />
        ...
        <div className='absolute opacity-0 group-hover:opacity-80 tansition duration-300 ease-in-out group-hover:bg-white h-16 w-16 md:w-28 md:h-28 rounded-full z-0'>
          ...
          <div className='flex h-full items-center justify-center'>
            ...
            <p className='text-xl md:text-2xl font-bold text-black opacity-100'>{skills.progress}%</p>
          </div>
        </div>
    </div>
  );
